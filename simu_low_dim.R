library(torch);library(glmnet) ;library(ncvreg);library(parallel);cl <- makeCluster(2)
...
for (ss in 1:50) {
  ...
  y_test <- ifelse(x_test %*% b_true + rnorm(n_test) >= 0, 1, 0) # + rnorm(n_test) # test labels

  # fit simple logistic loss pconfClassification 
  res_naive <- pconf_ncpen_noy(  num_epochs = 1000, lr = 0.01,penalty = 'L1',
                                x_train = x_train_p, x_test = x_test,   y_test = y_test,  r = r_train, 
                                lambda = 0  # setting lambda = 0 for unpenalized
  ) ; bhat_1 <-  res_naive$w
  pconfC_out[[ss]] = c(res_naive$accuracy, 'est'=  sum( (b_true - bhat_1)^2) , variable_select(b_true, bhat_1) )
  ...
  res_SCAD <- pconf_ncpen_noy(  num_epochs = 1000, lr = 0.01,penalty = 'SCAD',
                                x_train = x_train_p, x_test = x_test,   y_test = y_test,  r = r_train, 
  ...
  cv_ncv <- pconf_cv_lambda_ncpen(x = x_train_p, r = r_train,penalty = 'MCP', verbose = F, a =3,
                                  lambda_grid = 10^seq(-1, .01, length.out = 15) ) 
  res_MCP <- pconf_ncpen_noy(  num_epochs = 1000, lr = 0.01,penalty = 'MCP', a = 3, 
  ...
  cv_ncv <- pconf_cv_lambda_ncpen(x = x_train_p, r = r_train,penalty = 'L1', verbose = F,
                                  lambda_grid = 10^seq(-1, .01, length.out = 15) ) 
  res_L1 <- pconf_ncpen_noy(  num_epochs = 1000, lr = 0.01,penalty = 'L1',
                              x_train = x_train_p,   x_test = x_test,    y_test = y_test,
                              r = r_train, lambda =  cv_ncv$best_lambda
  ) ; bhat_L1 <-  res_L1$w
...
save.image('sim_n200p10_rX_.rda')
